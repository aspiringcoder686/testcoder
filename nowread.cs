...
namespace DynamicAnalyzer
{
    ...
    .ToList();
            ...
            // Console Output
            PrintHelper.Print("Methods Used", "Library\tNamespace\tMethod\tFileName", filteredMethods, r => new[] { r.Library, r.Namespace, r.Method, r.FileName });
            PrintHelper.Print("Properties Used", "Library\tNamespace\tProperty\tFileName", filteredProperties, r => new[] { r.Library, r.Namespace, r.Property, r.FileName });

            PrintHelper.Print("Fields Used", "Library\tNamespace\tField\tFileName",
      filteredFields, r => new[] { r.Library, r.Namespace, r.Field, r.FileName });

            PrintHelper.Print("Events Used", "Library\tNamespace\tEvent\tFileName",
                filteredEvents, r => new[] { r.Library, r.Namespace, r.Event, r.FileName });

            PrintHelper.Print("Object Creations", "Library\tNamespace\tType\tFileName",
                filteredObjects, r => new[] { r.Library, r.Namespace, r.Type, r.FileName });

            PrintHelper.Print("Attributes Used", "Library\tNamespace\tAttribute\tFileName",
                filteredAttributes, r => new[] { r.Library, r.Namespace, r.Attribute, r.FileName });

            PrintHelper.Print("Interfaces Used", "Library\tNamespace\tInterface\tFileName",
                filteredInterfaces, r => new[] { r.Library, r.Namespace, r.Interface, r.FileName });
        ...
        Func<T, string[]> rowSelector)
            {
                ...
                foreach (var item in items)
                {
                    ...
                    for (int col = 0; col < values.Length; col++)
                        ws.Cell(row, col + 2).Value = values[col];
                    row++;
                }
        ...
        {
            public static void Print<T>(string title, string header, IEnumerable<T> items, Func<T, string[]> rowSelector)
            {
                Console.WriteLine($"\n--- {title} ---");
                Console.WriteLine($"S.No.\t{header}");
                int sno = 1;
                foreach (var item in items)
                {
                    var values = rowSelector(item);
                    Console.WriteLine($"{sno++}\t{string.Join("\t", values)}");
                }
            }
        }
