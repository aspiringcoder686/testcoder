public static string ConvertToNativeSql(string hql, string entityName, string tableName, EntityDefinition entity)
{
    ...
    // Replace ? parameters with named ones
    ...
    // Replace IN (:param) with IN (@param)
    hql = MyRegexInClause().Replace(hql, match =>
    {
...
[GeneratedRegex(@"(\b\w+\b)\s+in\s+\(\s*:(\w+)\s*\)", RegexOptions.IgnoreCase, "en-US")]
private static partial Regex MyRegexInClause();
